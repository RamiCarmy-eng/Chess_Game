...
class ChessUltimate:
    ...
    def _explain_move_thorough(self, move: chess.Move, board: chess.Board,
        ...
        """
        Generate tips for every move — including WHY a different piece would be better.
        """
        tips = []
        ...
        if best is not None and best in board.legal_moves and board.is_capture(best) and not board.is_capture(move):
            ...
            if captured:
                ...
                tips.append(f"💰 Your {bp_name} on {chess.square_name(best.from_square)} could have captured "
        ...
        if best is not None and best in board.legal_moves:
            ...
            if test2.is_check() and not board_after.is_check():
                best_piece = board.piece_at(best.from_square)
                bp_name    = PNAME.get(best_piece.piece_type, "piece") if best_piece else "piece"
                tips.append(f"🎯 Your {bp_name} on {chess.square_name(best.from_square)} could have moved to "
        ...
        if best is not None and best in board.legal_moves:
            ...
            if test3.is_checkmate():
                best_piece = board.piece_at(best.from_square)
                bp_name    = PNAME.get(best_piece.piece_type, "piece") if best_piece else "piece"
                tips.append(f"👑 You missed CHECKMATE! {bp_name} to {chess.square_name(best.to_square)} was the winning move!")

        # ── 6. Suggest the better piece/move with explanation ─────────────────
        if best is not None and best in board.legal_moves and best != move and drop >= 10:
            ...
            if best_piece and my_piece and best_piece.piece_type != my_piece.piece_type:
                ...
                # Explain WHY the other piece is better
                reason = self._why_better(best, best_piece, board, board_after)
                tips.append(f"💡 Instead of the {my_name}, consider moving your {bp_name} "
                            f"from {from_sq} to {to_sq}. {reason}")
            elif best_piece and my_piece and best_piece.piece_type == my_piece.piece_type and best.to_square != move.to_square:
                bp_name = PNAME.get(best_piece.piece_type, "piece")
                to_sq   = chess.square_name(best.to_square)
                reason  = self._why_better(best, best_piece, board, board_after)
                tips.append(f"💡 The {bp_name} was right but {to_sq} is a stronger square. {reason}")

        # ── 7. King safety: moved king before castling ────────────────────────
        if piece_moved and piece_moved.piece_type == chess.KING:
            if board.has_castling_rights(chess.WHITE):
                tips.append("🏰 Moving your King early loses castling rights — try to castle first to stay safe!")
        ...
        if piece_moved and piece_moved.piece_type == chess.PAWN:
            ...
            if pawns_on_col >= 2:
                tips.append("📌 You now have doubled pawns on the same file — they can be hard to defend.")

        # ── 9. Opening: undeveloped pieces warning ────────────────────────────
        if move_count <= 14 and piece_moved:
            if piece_moved.piece_type == chess.QUEEN and move_count < 6:
                # Count undeveloped knights/bishops
                undeveloped = []
                for sq in chess.SQUARES:
                    p = board_after.piece_at(sq)
                    if p and p.color == chess.WHITE and p.piece_type in (chess.KNIGHT, chess.BISHOP):
                        if chess.square_rank(sq) == 0:  # still on back rank
                            undeveloped.append(PNAME.get(p.piece_type, "piece"))
                if undeveloped:
                    tips.append(f"⚠ Bringing your Queen out early is risky — develop your {undeveloped[0]} first!")

        # ── 10. Positive: good development ───────────────────────────────────
        if move_count <= 10 and piece_moved and not tips:
        ...
        if board.is_capture(move) and drop < 10 and not tips:
            ...
            if captured:
                ...
                if cap_val >= mover_val:
                    tips.append("💥 Nice capture! You traded well.")
    ...
    def _why_better(self, best: chess.Move, best_piece, board: chess.Board,
        ...
        """Return a plain-English reason why the best move is better."""
        ...
        if captured:
            ...
            if cap_val > mv_val:
                return f"It wins the opponent's {cap_name} for free — a great trade!"
            elif cap_val == mv_val:
                return f"It captures the opponent's {cap_name} — an even trade."

        # Does it give check?
        test = board.copy(); test.push(best)
        if test.is_check():
            return "It puts the opponent's King in check, forcing them to react!"

        # Does it control the centre?
        if to_sq in central:
            return "It places the piece on a powerful central square, controlling more of the board."
        if to_sq in near_centre and best_piece.piece_type in (chess.KNIGHT, chess.BISHOP):
            return "It develops a piece toward the centre — controlling key squares."

        # Is it development?
        if chess.square_rank(best.from_square) == 0 and best_piece.piece_type in (chess.KNIGHT, chess.BISHOP):
            return "It brings a new piece into the game — development is key in the opening!"

        # Attacks opponent's piece?
        test_board = board.copy(); test_board.push(best)
        for sq in chess.SQUARES:
            p = test_board.piece_at(sq)
            if p and p.color == chess.BLACK:
                if test_board.is_attacked_by(chess.WHITE, sq):
                    pname = self.PIECE_NAME.get(p.piece_type, "piece")
                    return f"It attacks the opponent's {pname} on {chess.square_name(sq)}!"

        # More squares controlled?
        if best_piece.piece_type == chess.KNIGHT:
            return "The Knight has more influence from that square."
        if best_piece.piece_type == chess.BISHOP:
            return "The Bishop has a longer diagonal from that square."
        if best_piece.piece_type == chess.ROOK:
            return "The Rook controls an open file from that square."

        return "It gives your piece a more active role in the position."
