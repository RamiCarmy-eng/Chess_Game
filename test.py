...
class ChessUltimate:
    ...
    def coach_speak(self, text: str):
        """עדכון ויזואלי מידי ודיבור בשיטה עוקפת"""
        # עדכון המסך (זה תמיד עובד)
        self.coach_text.config(state=tk.NORMAL)
        self.coach_text.delete("1.0", tk.END)
        self.coach_text.insert(tk.END, f"💡 Coach: {text}")
        self.coach_text.config(state=tk.DISABLED)

        # ניקוי אימוג'ים ידני (לפעמים unicodedata מוחק הכל)
        clean_text = text.replace('✅', '').replace('👌', '').replace('💡', '')

        # פתרון הדיבור: שימוש ב-Thread זמני שיוצר אובייקט חדש בכל פעם
        def _speak_task():
            import pyttsx3
            try:
                # אתחול מקומי בלבד בתוך ה-Thread
                engine = pyttsx3.init()
                engine.say(clean_text)
                engine.runAndWait()
                # שחרור המנוע כדי שלא יתקע את הקריאה הבאה
                del engine
            except Exception as e:
                print(f"TTS Error: {e}")
